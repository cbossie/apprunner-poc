...
WORKDIR /src
COPY ["AwsLambdaApplicationNormal.csproj", "AwsLambdaApplicationNormal/"]
RUN dotnet restore "AwsLambdaApplicationNormal/AwsLambdaApplicationNormal.csproj"

WORKDIR "/src/AwsLambdaApplicationNormal"
COPY . .
RUN dotnet build "AwsLambdaApplicationNormal.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "AwsLambdaApplicationNormal.csproj" \
...
Execute unit tests
```
cd "AwsLambdaApplicationNormal/test/AwsLambdaApplicationNormal.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "AwsLambdaApplicationNormal/src/AwsLambdaApplicationNormal"
